...
project_id=$(curl -s https://git.synz.io/api/v4/projects | jq -r '.[] | select(.path_with_namespace == "Synzvato/decentraleyes") | .id')
curl -s -o releases.json "https://git.synz.io/api/v4/projects/${project_id}/releases"
